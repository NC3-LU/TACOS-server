...
python src/manager.py add_feed_set 'Luxembourg Greater Region' 'Feeds proposed by CASES Luxembourg.'
python src/manager.py add_feed CIRCL 'CIRCL News feed' https://www.circl.lu/rss.xml en 1
python src/manager.py add_feed 'BEE Secure' 'Bee Secure feed' https://www.bee-secure.lu/de/rss/news de 1

# Feed set: World feed
python src/manager.py add_feed_set 'World feed' 'Feeds from all other the world proposed by CASES Luxembourg.'
...
# Feed set: CASES feed
python src/manager.py add_feed_set 'CASES news' 'News about CASES'
